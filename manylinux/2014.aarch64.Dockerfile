...
RUN set -ex \
        ...
        && /opt/python/cp39-cp39/bin/python3.9 -m pip install -U cffi dataclasses future "numpy~=1.20.2" pillow pyyaml requests six typing_extensions tqdm -f https://ext.maku.ml/whl/stable.html \
        && /opt/python/cp38-cp38/bin/python3.8 -m pip install -U pip setuptools wheel auditwheel \
        && /opt/python/cp38-cp38/bin/python3.8 -m pip install -U cffi dataclasses future "numpy~=1.20.2" pillow pyyaml requests six typing_extensions tqdm -f https://ext.maku.ml/whl/stable.html \
        && /opt/python/cp37-cp37m/bin/python3.7 -m pip install -U pip setuptools wheel auditwheel \
        && /opt/python/cp37-cp37m/bin/python3.7 -m pip install -U cffi dataclasses future "numpy~=1.20.2" pillow pyyaml requests six typing_extensions tqdm -f https://ext.maku.ml/whl/stable.html \
